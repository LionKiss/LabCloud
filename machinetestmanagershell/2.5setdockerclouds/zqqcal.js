...
var cloud = (function (){
  ...
  var machines = [
    {"IP":"172.16.2.16", "isMaster":true, "isPhysical": false, "MAC":"60A44CAd55FB"},
    {"IP":"172.16.2.36", "isMaster":false, "isPhysical": false, "MAC":"E0CB4EC8CF2E"},
    {"IP":"172.16.2.82", "isMaster":false, "isPhysical": false, "MAC":"000C2977d100"}
  ];  
  ...
  //默认 node1为主节点，node2为salve和registry节点，其余为salve节点
  //定义配置过程需要的变量名称
  var machineName = [{
    "master_hostname":hostnames[0].hostname.split(" ")[0],
	"KUBE_master_hostname":hostnames[0].hostname.split(" ")[0],
	"registryHostname":hostnames[0].hostname.split(" ")[1],
	"registryHostIP":machines[1].IP,
	"apiserverHostname":hostnames[0].hostname.split(" ")[0],
    "etcdHostname":hostnames[0].hostname.split(" ")[0],
	"apiserver_host":machines[0].IP,
	"kube_master_url":machines[0].IP
  }];
  //开机密码
  var pwd = "hadoop";

  //定义一个替换的函数
  function selfReplace(oldreplaceStr,replaceStr){
    var ForReading = 1, ForWriting = 2;
	var fso = new ActiveXObject("Scripting.FileSystemObject");
	var template = fso.OpenTextFile("zqqmkdockercloud.sh", ForReading);
	var tmp      = fso.OpenTextFile("zqqmkdockercloud.tmp.sh", ForWriting, true);
	var contentTemplate = template.ReadAll();
	tmp.Write(contentTemplate.replace(new RegExp(oldreplaceStr,'g'),replaceStr));
	template.Close();
	tmp     .Close();
    return "zqqmkdockercloud.tmp.sh";
  ...
  //这里的开机与关机在以及集群开机与集群关机，在特殊情况下使用，如重新唤醒主机或集群
  //关机命令
  ...
  //设置机器的主机名和IP地址：执行文件中host_hostname函数
  function setupIPhostname(){
	...
	for(var i =0;i<hostname.length;i++){
		...
		var fso = new ActiveXObject("Scripting.FileSystemObject");
		var template = fso.OpenTextFile("zqqmkdockercloud.sh", ForReading);
		var tmp      = fso.OpenTextFile("zqqmkdockercloud.tmp"+i+".sh", ForWriting, true);
		var contentTemplate = template.ReadAll();
		...
		var shell = WScript.CreateObject("WScript.Shell");
		//默认root用户权限直接启动
		shell.run("putty -m zqqmkdockercloud.tmp"+i+".sh -pw " +pwd+ " root@" +machines[i].IP, 1, true);
	}  
  }
  ...
  function only_init_master(){
	...
	var shell = WScript.CreateObject("WScript.Shell");
	//默认root用户权限直接启动
	shell.run("putty -m zqqmkdockercloud.tmp.sh -pw " +pwd+ " root@" +machines[0].IP, 1, true);
	//WScript.Echo("445554"+machines[i].IP);
  }
  //单独配置从节点
  ...
  function start_docker_cloud(){
	...
	var shell = WScript.CreateObject("WScript.Shell");
	//默认root用户权限直接启动
	shell.run("putty -m "+selfReplace("start_docker_cloud","startMasterSoftware\n")+" -pw " +pwd+ " root@" +machines[0].IP, 1, true);
	for(var i =1;i<machines.length;i++){
		//将文件通过putty进行远程执行
		var shell = WScript.CreateObject("WScript.Shell");
		//默认root用户权限直接启动
		shell.run("putty -m "+selfReplace("start_docker_cloud","startSlaveSoftware\n")+" -pw " +pwd+ " root@" +machines[i].IP, 1, true);
	}
  }
  //配给registry镜像本地库
  function registry_init_update(){
	...
	var tmp      = fso.OpenTextFile("zqqmkdockercloud.tmp.sh", ForWriting, true);
	var contentTemplate = template.ReadAll();
	tmp.Write(contentTemplate.replace(/registryinitupdate/g,"registry_init_update"+"\n"));
	template.Close();
	tmp     .Close();
	//将替换的文件通过putty进行远程执行
	var shell = WScript.CreateObject("WScript.Shell");
	//默认root用户权限直接启动
	shell.run("putty -m zqqmkdockercloud.tmp.sh -pw " +pwd+ " root@" +machineName[0].registryHostIP, 1, true); 
	start_docker_cloud();
  }
